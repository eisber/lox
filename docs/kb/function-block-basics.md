...
*[Icon Exclamation Mark Loxone]***Tip: **In the configuration software it is always the name of an object that is displayed. You could therefore use this field for information such as terminal arrangement, location in the consumer unit, cable ID or KNX address to help keep track of things. The description on the other hand is used only in the user interface (and only if set).
